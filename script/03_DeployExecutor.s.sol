...
contract DeployExecutor is Base {
    function run() public returns (address executor) {
        executor = computeCreate2Address(
            0, hashInitCode(type(LiquidityHub).creationCode, abi.encode(config.reactor, config.treasury))
        );

        if (executor.code.length == 0) {
            vm.broadcast(deployer);
            require(executor == address(new LiquidityHub{salt: 0}(config.reactor, config.treasury)));
        }
    }
}
