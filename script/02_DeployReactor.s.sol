...
contract DeployReactor is BaseScript {
    function run() public returns (address reactor) {
        ...
        console.logBytes32(hashInitCode(type(ExclusiveDutchOrderReactor).creationCode, abi.encode(permit2, address(0))));

        if (reactor.code.length == 0) {
            vm.broadcast();
            ExclusiveDutchOrderReactor deployed =
                new ExclusiveDutchOrderReactor{salt: salt}(IPermit2(permit2), address(0));
